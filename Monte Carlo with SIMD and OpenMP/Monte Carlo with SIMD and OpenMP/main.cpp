 ...
 *
 *
 * Finding the total volume of spheres using the Monte Carlo's method.
 * Also using SIMD and OpenMP
 *
 * 
...
void borders();
void borders(float * a, float& l, float& r);
void freeMemory();
int allocateMemory();
void sqrRadius();
...
double calculateVolume(unsigned totalSamples)
{
	...
	// So, I will make the radius it's square at the beginning.
	sqrRadius(); // TODO - with SIMD & threads...
	...
	//printf("xL=%f xR=%f yL=%f yR=%f zL=%f zR=%f\n", xL, xR, yL, yR, zL, zR);
#pragma omp parallel
{
	...
	for (; i < end; ++i)
	{
		...
		for (k = 0; k < n; ++k)
		{
			...
			//if ((x[k] - a)*(x[k] - a) + (y[k] - b)*(y[k] - b) + (z[k] - c)*(z[k] - c) <= r[k])
			{
				++inSphereSamples[idx];
				break;
			}
	...
	//float ratio = (float)inSphereSamples / (float)totalSamples;
	//float totalVolumeCube = xDist * yDist * zDist;

	//return ratio * totalVolumeCube;
...
// Note. Not a efficient reading from a file...
// No file validations!!
void readDataForShperes()
{
	ifstream in(name);
